...
namespace Log
{
	LOG getlog(wchar_t logfile[])
	{
		...
		LOG log;
		wcscpy_s(path, L"D:/KAA-2024/LOG/");
	...
	void WriteLog(LOG log)
	{
		...
		*log.stream << "\t\t\t\tПротокол [ " << temp << " ] " << std::endl;
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;

		std::cout << std::setfill('-') << std::setw(90) << '-' << endl;
		std::cout << "\t\t\t\tПротокол [ " << temp << " ] " << std::endl;
		std::cout << std::setfill('-') << std::setw(90) << '-' << endl;
	}
	void WriteIn(LOG log, In::IN in)
	{
		...
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;
		*log.stream << "\t\tКод на языке KAA-2024" << std::endl;
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;
		...
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;
		...
		std::cout << "Количество строк:\t" << in.lines + 1 << std::endl;
		std::cout << std::setfill('-') << std::setw(90) << '-' << endl;
	}
	void WriteParm(LOG log, Parm::PARM parm)
	{
		...
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;
		...
		std::cout << std::setfill('-') << std::setw(90) << '-' << endl;
		std::cout << "-in:\t" << in_text << "\n-out:\t" << out_text << "\n-log:\t" << log_text << endl;
		std::cout << "Дополнительно: " << (parm.idtable ? "-id  " : "") << (parm.lextable ? "-lex  " : "")
			<< (parm.tokens ? "-tokens  " : "") << (parm.rules ? "-rules " : "") << std::endl;
		std::cout << std::setfill('-') << std::setw(90) << '-' << endl;
	}
	...
	void WriteIDTableLog(IT::IdTable& idtable, LOG log)
	{
		...
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;
		*log.stream << "\tВсего идентификаторов: " << i - numberNoneId << std::endl;
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;
	}
	...
	{
		/*
		std::cout << "\t\t\tПРОМЕЖУТОЧНОЕ ПРЕДСТАВЛЕНИЕ КОДА" << std::endl;
		std::cout << std::setfill('-') << std::setw(90) << '-' << endl;
		std::cout << "0001\t";
		*/
		int number = 0;
		*log.stream << "\t\tПромежуточное представление кода" << std::endl;
		*log.stream << std::setfill('-') << std::setw(90) << '-' << endl;
		int i = 0;
