...
namespace Lexer
{
	int BinToDec(char* str) {
		...
		for (int i = 0; i < strlen(str); i++) { // Сама функция перевода
			Chislo += ((char)str[i] - '0') * pow(2, strlen(str) - j); //Алгоритм перевода степень двойки
			j++;
		}
		Chislo = Chislo * zn;
		return Chislo;
	...
	/*void DecToBin(int number, char* str) {
		int zn = 1;
		if (number < 0) {
			zn = -1;
			number = -number;
	...
	string DecToBin(int number) {
		...
		if (zn == -1) {
			binary = "1" + binary; // Добавление знака для отрицательных чисел
		}
		else {
			binary = "0" + binary;
		}
	...
	int DecToBin(char* str) {
		...
		for (int i = 0; i < strlen(str); i++) {
			Chislo += ((char)str[i] - '0') * pow(2, strlen(str) - j);
			j++;
		}
		Chislo = Chislo * zn;
		return Chislo;
	...
	LEX Analyze(In::IN in, Log::LOG log)
	{
		...
		for (int i = 0; word[i] != NULL; i++, indxLex++)											// проход по лексемам
		{
			...
			FST::FST fstNew(word[i], GRAPH_ANN);
			if (FST::execute(fstNew))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_ANN, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);
				continue;
			}

			FST::FST fstTypeInteger(word[i], GRAPH_INTEGER);
			...
			FST::FST fstTypeString(word[i], GRAPH_LINE);
			...
			FST::FST fstTypeCHR(word[i], GRAPH_SYMBOL);
			...
			FST::FST fstBreakl(word[i], GRAPH_STOP);
			...
			FST::FST fstSpeak(word[i], GRAPH_PRINT);
			...
			FST::FST fstWhere(word[i], GRAPH_IF);
			...
			if (FST::execute(fstIdentif))
			{
				...
				else
				{
					...
					if (idx != TI_NULLIDX)												// если такой идентификатор уже есть
					{
						LT::Entry entryLT = writeEntry(entryLT, LEX_ID, idx, line);
						LT::Add(lextable, entryLT);
						if (lextable.table[lextable.size - 3].lexema == LEX_ANN)
						{
				...
				if (findParm) {															// если параметр функции или процедуры
					...
					if (lextable.table[lextable.size - 2].lexema == LEX_TYPE && lextable.table[lextable.size - 3].lexema != LEX_ANN)
						Log::WriteError(log, Error::geterrorin(305, line, position));
					if (lextable.table[lextable.size - 2].lexema == LEX_ANN && lextable.table[lextable.size - 2].lexema != LEX_TYPE)
			...
			FST::FST fstLiteralInt(word[i], GRAPH_INTEGER_LITERAL);
			//if (IntToMinus((char*)word[i]) == -1)
			//	Log::WriteError(log, Error::geterrorin(325, line, -1));
			if (FST::execute(fstLiteralInt))
			{
				
					long long value = atoll((char*)word[i]);	// переводим значение целочисленного литерала из строкового в число
					
				
				if (value > INT_MAX || value < INT_MIN)
					throw Error::geterrorin(319, line, -1);
			...
			FST::FST fstLiteralCHR(word[i], GRAPH_SYMBOL_LITERAL);
			...
			FST::FST fstLiteralString(word[i], GRAPH_LINE_LITERAL);
			...
			//FST::FST fstLogOperator(word[i], GRAPH_LOG_OPERATOR_MOREEQ);
			//if (FST::execute(fstLogOperator)) {
			//	LT::Entry entryLT;

			//	switch (word[i][0]) {
			//	case '>=':
			//		entryIT = writeEntry(entryLT, LEX_LOGOPERATOR, LT_TI_NULLIDX, line - 1, LT::MORE);
			//		break;
			//	}
			//}
			...
			FST::FST fstEqual(word[i], GRAPH_ASSIGN);
			if (FST::execute(fstEqual))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_ASSIGN, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);
				continue;
			}
